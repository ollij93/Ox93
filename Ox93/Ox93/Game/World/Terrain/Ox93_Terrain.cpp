...
// Globals...
const u_int uOX93_TERRAIN_BIOME_SCALE = 128;
...
{
	Ox93_TerrainVertex* pxVertices = new Ox93_TerrainVertex[6 * uOX93_TERRAIN_NUMSIDEFACES * uOX93_TERRAIN_NUMSIDEFACES];
	u_int* puIndices = new u_int[6 * uOX93_TERRAIN_NUMSIDEFACES * uOX93_TERRAIN_NUMSIDEFACES];
	for (u_int u = 0; u < 6 * uOX93_TERRAIN_NUMSIDEFACES * uOX93_TERRAIN_NUMSIDEFACES; u++)
	{
		puIndices[u] = u;
	}

	for (u_int uX = 0; uX < uOX93_TERRAIN_NUMSIDEFACES; uX++)
	{
		for (u_int uZ = 0; uZ < uOX93_TERRAIN_NUMSIDEFACES; uZ++)
		{
			// POSITIONS
			{
				const float fFrac = Ox93_Math::Sqrt3 / 2.f;
				const float fX = static_cast<float>(uX) * fFrac * fOX93_TERRAIN_SCALE;
				const float fZ = (static_cast<float>(uZ) + 0.5f * (uX % 2)) * fOX93_TERRAIN_SCALE;

				const Ox93_Vector_3 xCorner0(fX, m_afHeightMap[uX][uZ], fZ);																					// 0, 0
				const Ox93_Vector_3 xCorner1(fX + fFrac * fOX93_TERRAIN_SCALE, m_afHeightMap[uX + 1][uZ], fZ + (0.5f - (uX % 2)) * fOX93_TERRAIN_SCALE);		// 1, 0
				const Ox93_Vector_3 xCorner2(fX, m_afHeightMap[uX][uZ + 1], fZ + fOX93_TERRAIN_SCALE);															// 0, 1
				const Ox93_Vector_3 xCorner3(fX + fFrac * fOX93_TERRAIN_SCALE, m_afHeightMap[uX + 1][uZ + 1], fZ + (1.5f - (uX % 2)) * fOX93_TERRAIN_SCALE);	// 1, 1

				if ( (uX % 2) == 0 )
				{
					pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 0].m_xPosition = xCorner0;
					pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 1].m_xPosition = xCorner2;
					pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 2].m_xPosition = xCorner1;

					pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 3].m_xPosition = xCorner3;
					pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 4].m_xPosition = xCorner1;
					pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 5].m_xPosition = xCorner2;
				}
				else
				{
					pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 0].m_xPosition = xCorner0;
					pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 1].m_xPosition = xCorner3;
					pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 2].m_xPosition = xCorner1;

					pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 3].m_xPosition = xCorner3;
					pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 4].m_xPosition = xCorner0;
					pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 5].m_xPosition = xCorner2;
				}
			}
			// NORMALS
			{
				const Ox93_Vector_3 xPos0 = pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 0].m_xPosition;
				const Ox93_Vector_3 xPos1 = pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 1].m_xPosition;
				const Ox93_Vector_3 xPos2 = pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 2].m_xPosition;

				const Ox93_Vector_3 xPos3 = pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 3].m_xPosition;
				const Ox93_Vector_3 xPos4 = pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 4].m_xPosition;
				const Ox93_Vector_3 xPos5 = pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 5].m_xPosition;


				const Ox93_Vector_3 xNormal1 = (xPos1 ^ xPos2) - (xPos0 ^ xPos2) + (xPos0 ^ xPos1);
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 0].m_xNormal = xNormal1;
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 1].m_xNormal = xNormal1;
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 2].m_xNormal = xNormal1;

				const Ox93_Vector_3 xNormal2 = (xPos4 ^ xPos5) - (xPos3 ^ xPos5) + (xPos3 ^ xPos4);
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 3].m_xNormal = xNormal2;
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 4].m_xNormal = xNormal2;
			...
			Ox93_Color xColor2;
			Ox93_Color xColor3;
			if (fBiomeValue0 > 0.8f)
			{
				xColor0 = { 0.5f, 0.5f, 0.5f, 1.f };
			}
			else
			{
				xColor0 = { 0.3f, 0.6f, 0.3f, 1.f };
			}
			if (fBiomeValue1 > 0.8f)
			{
				xColor1 = { 0.5f, 0.5f, 0.5f, 1.f };
			}
			else
			{
				xColor1 = { 0.3f, 0.6f, 0.3f, 1.f };
			}
			if (fBiomeValue2 > 0.8f)
			{
				xColor2 = { 0.5f, 0.5f, 0.5f, 1.f };
			}
			else
			{
				xColor2 = { 0.3f, 0.6f, 0.3f, 1.f };
			}
			if (fBiomeValue3 > 0.8f)
			{
				xColor3 = { 0.5f, 0.5f, 0.5f, 1.f };
			}
			else
			{
				xColor3 = { 0.3f, 0.6f, 0.3f, 1.f };
			}
			// Apply the colors
			if ((uX % 2) == 0)
			{
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 0].m_xColor = xColor0;
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 1].m_xColor = xColor2;
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 2].m_xColor = xColor1;

				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 3].m_xColor = xColor3;
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 4].m_xColor = xColor1;
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 5].m_xColor = xColor2;
			}
			else
			{
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 0].m_xColor = xColor0;
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 1].m_xColor = xColor3;
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 2].m_xColor = xColor1;

				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 3].m_xColor = xColor3;
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 4].m_xColor = xColor0;
				pxVertices[(uX * uOX93_TERRAIN_NUMSIDEFACES + uZ) * 6 + 5].m_xColor = xColor2;
			}
		}
	}
	...
	xVertexBufferDesc.ByteWidth = sizeof(Ox93_TerrainVertex) * 6 * uOX93_TERRAIN_NUMSIDEFACES * uOX93_TERRAIN_NUMSIDEFACES;
	...
	xIndexBufferDesc.ByteWidth = sizeof(u_int) * 6 * uOX93_TERRAIN_NUMSIDEFACES * uOX93_TERRAIN_NUMSIDEFACES;
...
bool Ox93_Terrain::GenerateRandom(int iTileX, int iTileZ, noise::module::Perlin xModule)
{
	for (u_int uX = 0; uX <= uOX93_TERRAIN_NUMSIDEFACES; uX++)
	{
		for (u_int uZ = 0; uZ <= uOX93_TERRAIN_NUMSIDEFACES; uZ++)
		{
			...
			const float fXFrac = (uX + iTileX * (float)uOX93_TERRAIN_NUMSIDEFACES) / (float)uOX93_TERRAIN_NUMSIDEFACES;
			const float fZFrac = (uZ + iTileZ * (float)uOX93_TERRAIN_NUMSIDEFACES) / (float)uOX93_TERRAIN_NUMSIDEFACES;

			// These are for the biome
	...
	m_xPosition = Ox93_Vector_3(iTileX * (float)uOX93_TERRAIN_NUMSIDEFACES * Ox93_Math::Sqrt3 * fOX93_TERRAIN_SCALE / 2.f, 0.f, iTileZ * (float)uOX93_TERRAIN_NUMSIDEFACES * fOX93_TERRAIN_SCALE);
...
bool Ox93_Terrain::GetClosestPointInRange(Ox93_Vector_3 xPos, float fRange, Ox93_Vector_3* pxResultPos) const
{
	...
	if (iClosestX < 0 || iClosestX > uOX93_TERRAIN_NUMSIDEFACES) { return false; }
	if (iClosestZ < 0 || iClosestZ > uOX93_TERRAIN_NUMSIDEFACES) { return false; }
	...
	// Central
	if (iClosestX > 0 && iClosestZ > 0 && iClosestX < uOX93_TERRAIN_NUMSIDEFACES && iClosestZ < uOX93_TERRAIN_NUMSIDEFACES)
	{
	...
	// neg X edge
	else if (iClosestZ > 0 && iClosestX < uOX93_TERRAIN_NUMSIDEFACES && iClosestZ < uOX93_TERRAIN_NUMSIDEFACES)
	{
		// Check the intersect point with each of the local triangles
	...
	// pos X edge
	else if (iClosestX > 0 && iClosestZ > 0 && iClosestZ < uOX93_TERRAIN_NUMSIDEFACES)
	{
		// Check the intersect point with each of the local triangles
		Ox93_Vector_3 xCorner[5];
		xCorner[0] = xClosestVertex;
	...
	// neg Z edge
	else if ((iClosestX % 2) == 0 && iClosestX > 0 && iClosestX < uOX93_TERRAIN_NUMSIDEFACES && iClosestZ < uOX93_TERRAIN_NUMSIDEFACES)
	{
		// Check the intersect point with each of the local triangles
	...
	// pos Z edge
	else if ((iClosestX % 2) == 0 && iClosestX > 0 && iClosestZ > 0 && iClosestX < uOX93_TERRAIN_NUMSIDEFACES)
	{
		// Check the intersect point with each of the local triangles
		Ox93_Vector_3 xCorner[6];
		xCorner[0] = xClosestVertex;
	...
	// neg X neg Z corner
	else if (iClosestX < uOX93_TERRAIN_NUMSIDEFACES && iClosestZ < uOX93_TERRAIN_NUMSIDEFACES)
	{
		// Check the intersect point with each of the local triangles
	...
	// neg X pos Z corner
	else if (iClosestZ > 0 && iClosestX < uOX93_TERRAIN_NUMSIDEFACES)
	{
		// Check the intersect point with each of the local triangles
		Ox93_Vector_3 xCorner[4];
		xCorner[0] = xClosestVertex;
	...
	// pos X neg Z corner
	else if (iClosestX > 0 && iClosestZ < uOX93_TERRAIN_NUMSIDEFACES)
	{
		// Check the intersect point with each of the local triangles
		Ox93_Vector_3 xCorner[3];
		xCorner[0] = xClosestVertex;
...
float Ox93_Terrain::GetHeightAtPoint(Ox93_Vector_3 xPos)
{
	...
	if (iX < 0 || iX > uOX93_TERRAIN_NUMSIDEFACES) { return 0.f; }
	if (iZ < 0 || iZ > uOX93_TERRAIN_NUMSIDEFACES) { return 0.f; }
...
Ox93_Vector_3 Ox93_Terrain::GetClosestCentre(Ox93_Vector_3 xPos)
{
	...
	Ox93_Assert(!(iX < 0 || iX > uOX93_TERRAIN_NUMSIDEFACES || iZ < 0 || iZ > uOX93_TERRAIN_NUMSIDEFACES), "Input Position out of range for GetHeightAtPoint");
	if (iX < 0 || iX > uOX93_TERRAIN_NUMSIDEFACES) { return Ox93_Math::ZeroVector3; }
	if (iZ < 0 || iZ > uOX93_TERRAIN_NUMSIDEFACES) { return Ox93_Math::ZeroVector3; }
